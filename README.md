...
1. An **8-transistor (8T) SRAM cell** with electrically isolated read and write paths
2. A **Multi-Logic Sense Amplifier (MLSA)** capable of producing multiple logic outputs (OR/NOR, AND/NAND, XOR/XNOR) from a single sensing circuit, using a tunable reference voltage
...
| A | B | RBL Voltage | OR (out1) | NOR (~out1) | XNOR (out2) |
|---|---|---|---|---|---|
| 0 | 0 | Vrbl > Vref | 0 | 1 | 1 |
| 0 | 1 | VDD/2 < Vrbl < Vref | 1 | 0 | 0 |
| 1 | 0 | VDD/2 < Vrbl < Vref | 1 | 0 | 0 |
| 1 | 1 | Vrbl < VDD/2 | 1 | 0 | 1 |
...
| Block | Description |
|---|---|
| 8T SRAM Cell | Storage core (M1–M4) + isolated read port (M6–M8); sized to balance read stability and performance |
| MLSA | Voltage-controlled latch + auxiliary logic for OR/NOR/AND/NAND/XOR/XNOR outputs, with a multiplexer for write-back routing |
| IMC Cell | Two 8T SRAM cells + one MLSA, forming a complete compute-in-memory unit, with a dedicated testbench for transient verification |

![8T SRAM Schematic](images/8T_SRAM_Sch.png)
![MLSA Schematic](images/MLSA_Sch.png)
![IMC Cell Schematic](images/IMC_Cell.png)
...
![8T SRAM Layout](images/8T_SRAM_Layout.png)
![MLSA Layout](images/MLSA_Layout.png)
![IMC Cell Layout](images/IMC_Layout.png)
...
![Bitline Discharge vs Vref](simulations/Bit-Line_Dis_Rate_Vref.png)
...
![XNOR Operation](simulations/MLSA_XNOR.png)
...
| Case | Power (µW) | Delay (pS) | PDP (fJ) |
|---|---|---|---|
| 00 | 33.80 | 170.90 | 5.77 |
| 01/10 | 52.196 | 18.35 | 0.95 |
| 11 | 133.1 | 16.75 | 2.29 |
...
| Case | Power (µW) | Delay (pS) | PDP (fJ) |
|---|---|---|---|
| 00 | 33.80 | 107.65 | 3.64 |
| 01/10 | 52.196 | 198.011 | 10.33 |
| 11 | 133.1 | 96.2 | 12.80 |
...
1. Pateliya, R. (2019). Static Noise Margin of 6T and 8T SRAM Cell in 28-nm CMOS. *ICACM*.
2. Wang, J., et al. (2019). A 28-nm compute SRAM with bit-serial logic/arithmetic operations for programmable in-memory vector computing. *IEEE JSSC*, 55(1), 76–86.
3. Thakur, M., & Mehra, R. An Energy-Efficient Sense Amplifier Using 180nm for SRAM.
4. Chen, J., et al. (2021). Analysis and design of reconfigurable sense amplifier for compute SRAM with high-speed compute and normal read access. *IEEE TCAS-II*, 68(12), 3503–3507.
5. Kumar, S. S., & Nayak, J. (2023). Efficient Implementation of Fast Half Adder using 8T SRAM for In-Memory Computing Applications. *MoSICom*, 648–652.
6. Rajput, A. K., & Pattanaik, M. (2020). Implementation of Boolean and arithmetic functions with 8T SRAM cell for in-memory computation. *INCET*, 1–5.
7. Chang, Y. J., & Chen, S. H. (2023). Multi-Logic Sense Amplifier (MLSA) Design for In-Memory Computing. *IEEE JETCAS*, 13(1), 371–381.

---

*Full academic report available in [`/report`](./report).*
